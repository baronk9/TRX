import React, { useState } from 'react';
import { Link } from 'react-router-dom';
import {
  LayoutGrid,
  Newspaper,
  Calendar,
  FileText,
  BarChart2,
  Activity,
  Shield,
  BarChart,
  BookOpen,
  MessageSquare,
  Cloud,
  Download,
  X,
  ChevronDown,
  ChevronRight
} from 'lucide-react';

interface SidebarProps {
  isOpen: boolean;
  onClose?: () => void;
}

export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  const [isMainOpen, setIsMainOpen] = useState(false);
  const [isProductsOpen, setIsProductsOpen] = useState(false);
  const [isOtherOpen, setIsOtherOpen] = useState(false);

  return (
    <>
      {/* Mobile Backdrop */}
      {isOpen && (
        <div
          className="fixed inset-0 bg-black/50 z-40 lg:hidden backdrop-blur-sm"
          onClick={onClose}
        />
      )}

      <aside
        className={`fixed lg:sticky top-0 left-0 z-50 flex-shrink-0 border-r border-white/10 bg-[#0B111A] h-screen overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'w-64 translate-x-0' : 'w-0 border-r-0 -translate-x-full'
          }`}
      >
        <div className="flex flex-col h-full w-64">
          <div className="p-4 flex justify-end">
            <button onClick={onClose} className="p-2 -mr-2 text-gray-400 hover:text-white transition-colors rounded-lg hover:bg-white/5">
              <X className="w-5 h-5" />
            </button>
          </div>

          <div className="flex-1 overflow-y-auto hidden-scrollbar py-6">
            {/* MAIN Section */}
            <div className="mb-6">
              <h4
                className="text-[10px] font-bold text-[#64748B] uppercase tracking-wider mb-2 px-3 flex items-center justify-between cursor-pointer hover:text-gray-300 transition-colors"
                onClick={() => setIsMainOpen(!isMainOpen)}
              >
                <span>Main</span>
                {isMainOpen ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
              </h4>

              {isMainOpen && (
                <nav className="space-y-1">
                  <a href="#" className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-[#1A222C] text-white border border-white/5 shadow-sm">
                    <LayoutGrid className="w-5 h-5 text-[#00d2ff]" />
                    <span className="font-medium text-sm">Dashboard</span>
                  </a>
                  <a href="#" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-[#94A3B8] hover:text-gray-200 hover:bg-white/5 transition-colors">
                    <Newspaper className="w-5 h-5" />
                    <span className="font-medium text-sm">News Feed</span>
                  </a>
                  <a href="#" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-[#94A3B8] hover:text-gray-200 hover:bg-white/5 transition-colors">
                    <Calendar className="w-5 h-5" />
                    <span className="font-medium text-sm">Calendar</span>
                  </a>
                  <a href="#" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-[#94A3B8] hover:text-gray-200 hover:bg-white/5 transition-colors">
                    <FileText className="w-5 h-5" />
                    <span className="font-medium text-sm">Reports</span>
                  </a>
                  <Link to="/cot-report" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-[#94A3B8] hover:text-gray-200 hover:bg-white/5 transition-colors">
                    <BarChart2 className="w-5 h-5" />
                    <span className="font-medium text-sm">COT Data</span>
                  </Link>
                  <Link to="/economic-indicators" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-[#94A3B8] hover:text-gray-200 hover:bg-white/5 transition-colors">
                    <Activity className="w-5 h-5" />
                    <span className="font-medium text-sm">Economic Indicators</span>
                  </Link>
                </nav>
              )}
            </div>

            {/* PRODUCTS Section */}
            <div className="mb-6">
              <h4
                className="text-[10px] font-bold text-[#64748B] uppercase tracking-wider mb-2 px-3 flex items-center justify-between cursor-pointer hover:text-gray-300 transition-colors"
                onClick={() => setIsProductsOpen(!isProductsOpen)}
              >
                <span>Products</span>
                {isProductsOpen ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
              </h4>

              {isProductsOpen && (
                <nav className="space-y-1">
                  <a href="#" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-[#94A3B8] hover:text-gray-200 hover:bg-white/5 transition-colors">
                    <Shield className="w-5 h-5" />
                    <span className="font-medium text-sm">Risk Checker</span>
                  </a>
                  <a href="#" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-[#94A3B8] hover:text-gray-200 hover:bg-white/5 transition-colors">
                    <BarChart className="w-5 h-5" />
                    <span className="font-medium text-sm">Developer API</span>
                  </a>
                </nav>
              )}
            </div>

            {/* OTHER Section */}
            <div>
              <h4
                className="text-[10px] font-bold text-[#64748B] uppercase tracking-wider mb-2 px-3 flex items-center justify-between cursor-pointer hover:text-gray-300 transition-colors"
                onClick={() => setIsOtherOpen(!isOtherOpen)}
              >
                <span>Other</span>
                {isOtherOpen ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
              </h4>

              {isOtherOpen && (
                <nav className="space-y-1">
                  <a href="#" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-[#94A3B8] hover:text-gray-200 hover:bg-white/5 transition-colors">
                    <BookOpen className="w-5 h-5" />
                    <span className="font-medium text-sm">Learn more</span>
                  </a>
                  <a href="#" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-[#94A3B8] hover:text-gray-200 hover:bg-white/5 transition-colors">
                    <MessageSquare className="w-5 h-5" />
                    <span className="font-medium text-sm">Contact us</span>
                  </a>
                </nav>
              )}
            </div>
          </div>

          {/* Bottom Card */}
          {isOtherOpen && (
            <div className="p-4 mt-auto">
              <div className="bg-[#121820] border border-white/10 rounded-xl p-4">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2 text-white">
                    <Cloud className="w-5 h-5 text-gray-400" />
                    <span className="font-bold text-sm">Personal</span>
                  </div>
                  <button className="px-3 py-1 rounded-full border border-white/20 text-xs font-medium text-white hover:bg-white/10 transition-colors">
                    Upgrade
                  </button>
                </div>

                <div className="mb-2">
                  <div className="h-1.5 w-full bg-[#1A222C] rounded-full overflow-hidden">
                    <div className="h-full bg-[#00d2ff] rounded-full" style={{ width: '63.3%' }}></div>
                  </div>
                </div>
                <p className="text-[11px] text-[#94A3B8] mb-4">19 Analyses used of 30</p>

                <button className="w-full flex items-center justify-center gap-2 text-sm font-medium text-[#94A3B8] hover:text-white transition-colors">
                  <Download className="w-4 h-4" />
                  Get the app
                </button>
              </div>
            </div>
          )}
        </div>
      </aside>
    </>
  );
}
